//<noinclude>
//[[WP:POP]] ver. [[User:A2569875]]， add 
//1. otherwiki support. e.g. in zhwiki, allow you preview otherwiki, e.g. enwiki, jawiki, wikidata, zh.moegirl... etc.
//2. first image of section.
//3. wikidata support.
//4. javascript code preview. e.g. User:Xxx/Xxx.js
//BUG:
//wiki site data in user link is missing. e.g. in zhwiki, link of [[:en:User:Xxx]]'s Contributions is https://zh.wikipedia.org...
//wiki site data in special link is missing. e.g. in zhwiki, link of move page [[:en:Xxx]] is https://zh.wikipedia.org...
var mwRegExpescape;
$(function () {
//////////////////////////////////////////////////
// Globals
...
function mouseOverWikiLink2(a, evt) {
	...
	var site_s = '';
	if(!!get_site){if(!!get_site[1]) site_s = get_site[1];}
	checkSiteCatch(site_s, function(){
...
function getApiPath(site_name){
	var defaultpath = (pg.wiki.titlebasehead + site_name + pg.wiki.titlebasetail);
	for(var it in pg.wiki.apibaseData){ 
		if(pg.wiki.apibaseData[it].regex.test(site_name)){
			return pg.wiki.titlebasehead + site_name + pg.wiki.apibaseData[it].titletail;
		}
	}
	return defaultpath;
}

function getIndexPath(site_name){
	var defaultpath = (pg.wiki.titlebasehead + site_name + pg.wiki.botInterfacePath);
	for(var it in pg.wiki.titlebaseData){ 
		if(pg.wiki.titlebaseData[it].regex.test(site_name)){
			return pg.wiki.titlebasehead + site_name + pg.wiki.titlebaseData[it].titletail.split('?')[0];
		}
	}
	return defaultpath;
}

function getArticlePath(site_name){
	var defaultpath = (pg.wiki.titlebasehead + site_name + pg.wiki.articlePath);
	for(var it in pg.wiki.articlebaseData){ 
		if(pg.wiki.articlebaseData[it].regex.test(site_name)){
			return pg.wiki.titlebasehead + site_name + pg.wiki.articlebaseData[it].titletail;
		}
	}
	return defaultpath;
}
...
//</NOLITE>
	...
	var site_r = getSpecialRegexBySite(article.site);
	if(!site_r)site_r=pg.re;
...
//<NOLITE>
		...
		if(params.not_diff){ 
			article.oldid = params.oldid;
			article.value = '';
			loadAPIPreview('revision', article, a.navpopup); 
		}
		else loadDiff(article, oldid, diff, a.navpopup);
	} else if ( history ) {
		loadAPIPreview('history', article, a.navpopup);
	} else if ( site_r.contribs.test(a.href) ) {
		loadAPIPreview('contribs', article, a.navpopup);
	} else if ( site_r.backlinks.test(a.href) ) {
		loadAPIPreview('backlinks', article, a.navpopup);
	} else if ( site_r.loglinks.test(a.href) ) {
		loadAPIPreview('loglinks', article, a.navpopup);
	} else if ( site_r.specialabusefilter.test(a.href) ) {
		loadAPIPreview('specialabusefilter', article, a.navpopup);
...
function loadPreviewFromRedir(redirMatch, navpop, new_site) {
	redir_site = navpop.article.site;
	if(typeof new_site != "undefined")redir_site = new_site;
	// redirMatch is a regex match
	var target = new Title().fromWikiText(redirMatch[2], redir_site);
...
function insertWikiaPreview(download, olddata) {
	if (!download.owner) { return; }
	var wikia_struct_str = download.data;
	var wikia_text = '';
	if(olddata.thumbnail){
		var popupImage = document.getElementById("popupImg" + download.owner.idNumber);
		if (!popupImage) {
		...
		popupImage.src=olddata.thumbnail;
		var popupImage_a=document.getElementById("popupImageLink" + download.owner.idNumber);
		popupImage_a.onclick=toggleSize;
		popupImage_a.title=popupString('Toggle image size');
	}

	try { 
		var wikia_struct = getJsObj(wikia_struct_str); 
		for(var i = 0; i < wikia_struct.sections.length; ++i){
			if(i != 0)wikia_text += "== " + wikia_struct.sections[i].title + " ==\n";
			for(var j=0; j<wikia_struct.sections[i].content.length; ++j){
				wikia_text += wikia_struct.sections[i].content[j].text + "\n\n";
			}
		}
	}catch(ex){}
	download.data = wikia_text;
...
function insertPreview(download) {
	...
	var soft_otherwiki_redirMatch = RegExp("\{\"title\":(\"([^\\\"]|\\[^\\])+\"),\"iw\":(\"([^\\\"]|\\[^\\])+\")\}").exec(download.data);

	if (soft_otherwiki_redirMatch !== null){
		language_code = soft_otherwiki_redirMatch[3].substring(1, soft_otherwiki_redirMatch[3].length - 1);
		wiki_tail = RegExp("^[^\\.]+(\\..+)$").exec(download.owner.article.site);
		if(wiki_tail !== null)wiki_tail = wiki_tail[1];
		redirMatch = pg.re.redirect.exec("#redirect [[" + soft_otherwiki_redirMatch[1].substring(language_code.length + 2, soft_otherwiki_redirMatch[1].length - 1) + "]]\n");
		if(language_code != 'common'){
			download.owner.redir = 0;
			loadPreviewFromRedir(redirMatch, download.owner, language_code + wiki_tail);
		}
		return;
	}else if (download.owner.redir === 0 && redirMatch) {
...
function checkSiteCatch(the_site, callback){
	if(the_site === '' || RegExp("\\.wikia\\.com").test(the_site)){
		if(typeof callback === 'function'){
			callback();
		}
		return;
	}
	if(typeof pg.wiki.sitecatch[the_site] == 'undefined'){
		pg.wiki.sitecatch[the_site] = {};
		mw.loader.using( 'mediawiki.ForeignApi' ).done( function () {

			var api_url = getApiPath(the_site);

		    var api = new mw.ForeignApi( api_url, { async:false } );
		    api.get( {
		        action: 'query',
		        meta: 'siteinfo',
		        siprop: ['namespaces', 'namespacealiases', 'specialpagealiases'],
		        formatversion: '2'
		    }).done( function ( data ) {
		    	pg.wiki.sitecatch[the_site].namespace = {};
		    	pg.wiki.sitecatch[the_site].namespaceid = {};
		    	pg.wiki.sitecatch[the_site].specialPageAliases = {};
				pg.wiki.sitecatch[the_site].specialPageName = {};
		        for(i in data.query['namespaces']){
		        	pg.wiki.sitecatch[the_site].namespace[i] = data.query['namespaces'][i].name;
		        	pg.wiki.sitecatch[the_site].namespaceid[data.query['namespaces'][i].name.split(' ').join('_').toLowerCase()] = parseInt(i);
		        }
		        for(i in data.query.namespacealiases){
		        	pg.wiki.sitecatch[the_site].namespaceid[data.query.namespacealiases[i].alias.split(' ').join('_').toLowerCase()] = parseInt(data.query.namespacealiases[i].id);
		        }
		    	setSpecialPageBase(pg.wiki.specialNeed, data.query['specialpagealiases'], the_site);
		        if(typeof callback === 'function'){
		        	callback();
		        }
		    } );
...
function insertPreviewNow(download) {
	...
	var o = null, wikidata_c=null, is_wikidata=false, has_inner=false, filled_data = false;
	try { 
		o=getJsObj(wikiText); 
		var ns = o.query.pages[0].ns;
		o=o.query.pages[0].revisions[0].content; 
		if(ns == 828 || ns == 2){
			wiki = "!!!code!!!" + o;
			wikiText = wiki;
			download.data = wiki;
	...
	if(!filled_data){
		try { wikidata_c=getJsObj(o); 
			if(typeof wikidata_c !== typeof 1){
				if(!wikidata_c["flow-workflow"])is_wikidata=true;
				else{
					FlowPreviewHTML(download.owner.article, download);
					return;
					//wikiText = "[[WP:Flow]]討論頁不支援預覽。";
					//download.data = wikiText;
				}
			}
		} catch(ex_err2) {};
		if(wikidata_c==1)is_wikidata=false;
		if(has_inner && !is_wikidata){
			wiki = o;
		}
		if(is_wikidata){
			img_str = '';
			try { 
				get_images=getJsObj(wikiText); 
				img_str = "[[" + get_images.query.pages[0].images[0].title + "]]";
			}catch(ex_err3){ }
			var pg_name = popupString('NoTag');
			var pg_des = '';
			var pg_links = '';
			var pg_tail = JSON.stringify(wikidata_c).replace( /\"/g, '' );
			if(typeof wikidata_c.sitelinks != 'undefined'){
				if(typeof wikidata_c.sitelinks['zhwiki'] != 'undefined'){pg_links += " !!!anchorheadbegin!!!" + 
					"!!!zh!!!" + encodeURI(wikidata_c.sitelinks['zhwiki'].title) + " !!!anchorheadend!!!zh!!!anchortail!!! ";}
				if(typeof wikidata_c.sitelinks['enwiki'] != 'undefined'){pg_links += " !!!anchorheadbegin!!!" + 
					"!!!en!!!" + encodeURI(wikidata_c.sitelinks['enwiki'].title) + " !!!anchorheadend!!!en!!!anchortail!!! ";}
				if(typeof wikidata_c.sitelinks['jawiki'] != 'undefined'){pg_links += " !!!anchorheadbegin!!!" + 
					"!!!ja!!!" + encodeURI(wikidata_c.sitelinks['jawiki'].title) + " !!!anchorheadend!!!ja!!!anchortail!!! ";}
				if(pg_links != '') pg_links = "!!!subhead!!!" + pg_links + "!!!subtail!!!";
			}
			if(typeof wikidata_c.claims != 'undefined'){
				claims = wikidata_c.claims;
				claims_data = '';
				for (c_i in claims) {
					claims_data += "[[Property:" + c_i + '|' + c_i + "]] : { ";
					for (c_j in claims[c_i]) {
						if(claims[c_i][c_j] && claims[c_i][c_j].mainsnak  && claims[c_i][c_j].mainsnak.datavalue){
							datavalue = claims[c_i][c_j].mainsnak.datavalue;
							switch(datavalue.type){
								case "wikibase-entityid":
									if(datavalue.value["entity-type"] == "item"){
										claims_data += "[[" + datavalue.value.id + "]] ";
									}else if(datavalue.value["entity-type"] == "property"){
										claims_data += "[[Property:" + datavalue.value.id + '|' + datavalue.value.id + "]] ";
									}
									break;
								case "string":
									claims_data += datavalue.value + " ";
									break;
								case "quantity":
									claims_data += datavalue.value.amount + " !!!anchorheadbegin!!!" + 
									( (datavalue.value.unit === '1') ? (pg.wiki.articlebase + '/1' ) :
									datavalue.value.unit.replace(/\/entity\//g, "/wiki/").replace(/http:\/\//g, "https://")
									) + "!!!anchorheadend!!!" + popupString("NumberUnit") + "!!!anchortail!!! ";
									break;
								default:
									break;
							}
			...
			if(typeof wikidata_c.labels != 'undefined'){
				if(typeof wikidata_c.labels["zh-tw"] != 'undefined'){
					pg_name = wikidata_c.labels["zh-tw"].value;
				}else if(typeof wikidata_c.labels["zh-hant"] != 'undefined'){
					pg_name = wikidata_c.labels["zh-hant"].value;
				}else if(typeof wikidata_c.labels["zh-hans"] != 'undefined'){
					pg_name = wikidata_c.labels["zh-hans"].value;
				}else if(typeof wikidata_c.labels["zh-cn"] != 'undefined'){
					pg_name = wikidata_c.labels["zh-cn"].value;
				}else if(typeof wikidata_c.labels.zh != 'undefined'){
					pg_name = wikidata_c.labels.zh.value;
				}else if(typeof wikidata_c.labels.en != 'undefined'){
					pg_name = wikidata_c.labels.en.value;
				}
			}
			if(typeof wikidata_c.descriptions != 'undefined'){
				if(typeof wikidata_c.descriptions["zh-tw"] != 'undefined'){
					pg_des = wikidata_c.descriptions["zh-tw"].value;
				}else if(typeof wikidata_c.descriptions["zh-hant"] != 'undefined'){
					pg_des = wikidata_c.descriptions["zh-hant"].value;
				}else if(typeof wikidata_c.descriptions["zh-hans"] != 'undefined'){
					pg_des = wikidata_c.descriptions["zh-hans"].value;
				}else if(typeof wikidata_c.descriptions["zh-cn"] != 'undefined'){
					pg_des = wikidata_c.descriptions["zh-cn"].value;
				}else if(typeof wikidata_c.descriptions.zh != 'undefined'){
					pg_des = wikidata_c.descriptions.zh.value;
				}else if(typeof wikidata_c.descriptions.en != 'undefined'){
					pg_des = wikidata_c.descriptions.en.value;
				}
			}
			wiki = "!!!iswikidata!!!==" + pg_name + pg_links + "==\n" + pg_des + "!!!newline!!!!!!newline!!!" + claims_data + img_str + "\n<br/>\n==Datas==\n<br/>\n" + pg_tail;
			wikiText = wiki;
			download.data = wiki;
		}
	}
	...
	var missing_article = false;
	if(download.missing_title){
		navpop.originalArticle = download.missing_title;
		download.missing_title = null;
		missing_article = true;
	}
	var art=navpop.redirTarget || navpop.originalArticle;
	art.missing_article = missing_article;
//<NOLITE>
	...
	if (art.namespaceId()==pg.nsImageId) { imagePage=art.toString(); }
		...
		get_image_wikitext = wikiText;
		anchor_flag = false;
		if(download.owner && download.owner.redirTarget && download.owner.redirTarget.anchor){
			var anchor_wikitext=anchorize(wikiText, download.owner.redirTarget.anchorString());
			if(typeof anchor_wikitext != 'undefined'){get_image_wikitext = anchor_wikitext; anchor_flag = true; }
		}else if(download.owner && download.owner.article && download.owner.article.anchor){
			var anchor_wikitext=anchorize(wikiText, download.owner.article.anchorString());
			if(typeof anchor_wikitext != 'undefined'){get_image_wikitext = anchor_wikitext; anchor_flag = true; }
		}
		imagePage=getValidImageFromWikiText(get_image_wikitext, download.owner.article.site); 
		if(!imagePage && wikiText != anchor_wikitext && anchor_flag)imagePage=getValidImageFromWikiText(wikiText, download.owner.article.site);
	}
	if(imagePage) { loadImage(Title.fromWikiText(imagePage, art.site), navpop); }
//</NOLITE>
...
function insertArticlePreview(download, art, navpop) {
	if (download && typeof download.data == typeof ''){
		...
		if(art.missing_article){
			var popupElement=document.getElementById("navpopup_maindiv" + (navpop.idNumber - 1));
			popupElement.innerHTML = popupElement.innerHTML.replace(/！！！PageName！！！/g, art.value).replace(/%EF%BC%81%EF%BC%81%EF%BC%81PageName%EF%BC%81%EF%BC%81%EF%BC%81/g, 
				art.urlString());
		}
	}
}
...
function prepPreviewmaker(data, article, navpop) {
	...
	//pg.wiki.articlebase
	var new_base = getArticlePath(article.site);

	var urlBase=joinPath([new_base, article.urlString()]);
...
function anchorize(d, anch) {
	if (!anch) { return d; }
	var reg_str = '(?:=+\\s*' + literalizeRegex(anch).replace(/[_ ]/g, '[_ ]') + 
		'\\s*=+|\\{\\{\\s*'+getValueOf('popupAnchorRegexp') + 
		'\\s*(?:\\|[^|}]*)*?\\s*'+literalizeRegex(anch)+'\\s*(?:\\|[^}]*)?}})'
	var anchRe=RegExp(reg_str);
...
pg.structures.nostalgia.popupTopLinks=function(x)  {
	...
	// misc links
	str += '<br><<whatLinksHere|shortcut=l>>*<<relatedChanges|shortcut=r>>*<<purgeLink>>*<<logLink>>';
...
pg.structures.fancy.popupOtherLinks=function(x) {
	...
	var normal='<<whatLinksHere|shortcut=l|links here>>*<<relatedChanges|shortcut=r|related>>*<<purgeLink>>*<<logLink>>';
...
pg.structures.menus.popupTopLinks = function (x, shorter) {
	...
	var afhist='<<afhistory|shortcut=h>>';
	...
	var purgeLink='<<purgeLink|purge Link>>';
	var logLink='<<logLink|log Link>>';
	...
	if (getValueOf('popupActionsMenu')) {
		s.push( '<<mainlink>>*<<wikisitelink>>*' + dropdiv + menuTitle('actions'));
	} else {
		s.push( dropdiv + '<<mainlink>>*<<wikisitelink>>');
	}
	s.push( '<menu>'); 
	s.push( ((!x.article.isSpecialabusefilter) ? editRow : '') + 
		((!x.article.iswikia) ? 
			((x.article.isSpecialabusefilter) ? afhist : markPatrolled + newTopic + hist + lastedit + thank)
		:hist) );
	if (!shorter) { s.push(jsHistory); }
	if (!x.article.iswikia && !x.article.isSpecialabusefilter) s.push( move + linkshere + related + purgeLink + logLink);
	if (!shorter) { s.push(nullPurge + search); }
	if (!shorter) { s.push(viewOptions); }
	if (!x.article.iswikia && !x.article.isSpecialabusefilter) s.push('<hr />' + watch + protectDelete);
	s.push('<hr />' +
		   ((!x.article.iswikia && !x.article.isSpecialabusefilter) ? ('if(talk){<<article|shortcut=a|view article>><<editArticle|edit article>>}' +
	...
	if (!x.article.iswikia && !x.article.isSpecialabusefilter) s.push('<<userTalk|shortcut=t|user talk>><<editUserTalk|edit user talk>>' +
	...
	if (!x.article.iswikia && !x.article.isSpecialabusefilter) s.push('if(admin){<menurow><<unblock|unblockShort>>|<<block|shortcut=b|block user>></menurow>}');
	if (!x.article.iswikia && !x.article.isSpecialabusefilter) s.push('<<blocklog|shortcut=B|block log>>');
	s.push('</menu>'  + enddiv + '}');
...
function autoEdit() {
	setupPopups( function () {
		...
		if (rvid) {
			...
			//var url = pg.wiki.titlebasehead + article.site + pg.wiki.apibasetail + '?action=query&format=json&formatversion=2&prop=revisions&revids='+rvid;
			//if (pg.re.specialwikisitelink.test(article.site)){
			//	url = (pg.wiki.titlebasehead + article.site + '/api.php?action=query&format=json&formatversion=2&prop=revisions&revids='+rvid);
			//}
...
Downloader.prototype.setTarget = function () {
	...
	var is_done = false;
	url_pre = url.split("?")[0];
	url_post = "action=query&meta=tokens&format=json&origin=" + encodeURI(pg.wiki.titlebasehead + pg.wiki.sitebase);
	url_pre += "?" + url_post;
	var setting = false;
	if(url.indexOf(pg.wiki.sitebase) < 0){
		try {
			this.withCredentials = true;
			this.http.open('POST', url_pre, false);
			this.http.setRequestHeader( 'Api-User-Agent', pg.misc.userAgent );
			this.http.send(url_post);
		}
		catch (somerr) {
			if(this.url.indexOf("&token=") <= 0) this.url += "&origin=" + encodeURI(pg.wiki.titlebasehead + pg.wiki.sitebase);
			setting = true;
		}
		if( this.http.status == 200 ){
			try{
				the_data = JSON.parse(this.http.responseText);

				if(this.url.indexOf("&token=") <= 0) this.url += "&token=" + the_data.query.tokens.csrftoken + "&origin=" + encodeURI(pg.wiki.titlebasehead + pg.wiki.sitebase);
				setting = true;
			}catch(somerr){ /* ignore it */ };
		}
	}
	this.http.open(this.method, this.url, this.async);
	if(!setting)this.http.setRequestHeader( 'Api-User-Agent', pg.misc.userAgent );
};
...
Insta.convert = function(wikiText)
{
	wiki = wikiText;
...
function wiki2html(txt,baseurl,siteurl) {
	Insta.conf.baseUrl = baseurl;
	Insta.conf.paths.articles = siteurl;
...
function countImages(wikiText) {
	...
	return (wikiText.parenSplit(reim).length - 1) / (pg.re.imageBracketCount + 1);
}
...
Title.prototype.checkWikia=function(){
	return RegExp("[a-zA-Z0-9\\-\\.]+\\.wikia\\.com").test(this.site);
}
Title.prototype.fromURL=function(h) {
	...
	if(RegExp("//[a-zA-Z0-9\\-\\.]+\\.wikia\\.com").test(h)){
		var redirsite_test = RegExp("//www\\.wikia\\.com/wiki/c\\:([^\\:]+)\\:([^?]+)").exec(h);
		var wikiasite_test = RegExp("//([^\n]+)\\.wikia\\.com/wiki/([^?]+)").exec(h);
		var test_result = (redirsite_test !== null)?redirsite_test:wikiasite_test;
		this.value = test_result[2];
		this.iswikia = true;
		this.site = test_result[1] + ".wikia.com";
		this.wikia = test_result[1];
		return this;
	}
	if(!(RegExp("//" + literalizeRegex(pg.wiki.sitebase + '/')).test(h))){
		var complete = false;
		if(RegExp("index\\.php").test(h)){
			for(var it in pg.wiki.titlebaseData){ var element = pg.wiki.titlebaseData[it];
				for(var jt in element.sites){ var site_name = element.sites[jt];
					var rexp = literalizeRegex(site_name);
					if(RegExp("//[a-z\\-]+\\." + rexp + '/').test(h)){
						h_temp=h_temp.replace(RegExp("//([a-z\\-]+\\.)" + rexp + element.titletail.split('?')[0],'g'),
															"//$1" + site_name + pg.wiki.botInterfacePath)
						complete = true;
						break;
					}
				}
			...
			for(var it in pg.wiki.articlebaseData){ var element = pg.wiki.articlebaseData[it];
				for(var jt in element.sites){ var site_name = element.sites[jt];
					var rexp = literalizeRegex(site_name);
					if(RegExp("//[a-z\\-]+\\." + rexp + '/').test(h)){
						h_temp=h_temp.
							replace(RegExp("//([a-z\\-]+\\.)" + rexp + "/index",'g'),					"//$1" + site_name + "wwww/index").
							replace(RegExp("//([a-z\\-]+\\.)" + rexp + element.titletail + '/','g'),	"//$1" + site_name + pg.wiki.articlePath + '/').
							replace(RegExp("//([a-z\\-]+\\.)" + rexp + "wwww/index",'g'), 			"//$1" + site_name + "/index");
						complete = true;
						break;
					}
				}
	...
	h_temp = h_temp.replace(/Special(:|\%3A)EntityPage(\/|\%2F)/g, '');

	var the_site = pg.wiki.sitebase;
	var m = pg.re.main.exec(h_temp);

	if(m === null) { the_site=pg.wiki.sitebase; }
	else {
		the_site = m[1]
	}
	this.site = the_site;
	checkSiteCatch(the_site);

	var contribs=getSpecialRegexBySite(the_site).contribs.exec(h_temp);
	if (contribs) {
	...
	var email=getSpecialRegexBySite(the_site).email.exec(h_temp);
	if (email) {
		this.setUtf(this.decodeNasties(mw.config.get('wgFormattedNamespaces')[pg.nsUserId] + ':' + new Title(email[3]).stripNamespace()));
		return this;
	}

	var backlinks=getSpecialRegexBySite(the_site).backlinks.exec(h_temp);
	if (backlinks) {
		this.setUtf(this.decodeNasties(new Title(backlinks[3])));
		return this;
	}

	var specialabusefilters=getSpecialRegexBySite(the_site).specialabusefilter.exec(h_temp);
	if (specialabusefilters) {
		this.setUtf(this.decodeNasties(new Title(specialabusefilters[2])));
		this.isSpecialabusefilter = true;
		return this;
	}
	
	var loglinks=getSpecialRegexBySite(the_site).loglinks.exec(h_temp);
	var islogpage = false;
	if (loglinks) {
		//4 = user (pre)
		//8 = page
		//12 = user (post)
		if(typeof loglinks[4] !== 'undefined' || typeof loglinks[12] !== 'undefined'){
			username = ((typeof loglinks[4] !== 'undefined') ? loglinks[4] : loglinks[12]);
			if(typeof loglinks[3] !== 'undefined' && !(RegExp("user").test(loglinks[3] || ''))){
				var check_ltype = RegExp("&type=([^&]+)").exec(h_temp);
				var check_luser = RegExp("&user=([^&]+)").exec(h_temp);
				if(check_ltype){
					this.logtype = check_ltype[1];
				}else if(window.a2569875PopUp.Module.LogEventTypes){
					if(window.a2569875PopUp.Module.LogEventTypes.includes(username)){
						this.logtype = username; //不是用戶名
						username = loglinks[12] || ((check_luser) ? check_luser[1] : null) || '';
					}
				}
			}
			if(typeof username !== 'undefined' && username !== '') this.byUser = (this.decodeNasties(username));
		}
		if(typeof loglinks[8] !== 'undefined' && loglinks[8] !== '') {
			this.setUtf(this.decodeNasties(new Title(loglinks[8])));
			return this;
		}
		islogpage = true;
	}

	var permanentlink=getSpecialRegexBySite(the_site).permanentlink.exec(h_temp);
	if (permanentlink) {
		this.isoldid = true;
		this.setUtf(this.decodeNasties(new Title("！！！PageName！！！")));
		return this;
	}

	//A dummy title object for a Special:Diff link.
	var specialdiff=getSpecialRegexBySite(the_site).specialdiff.exec(h_temp);
	if (specialdiff) {
	...
	if(m === null) { this.value=null; }
		...
		var fromBotInterface = /[?](.+[&])?title=/.test(h_temp);
		this.site = m[1]

		if (fromBotInterface) {
			m[2 + 1]=m[2 + 1].split('+').join('_');
		}
		var extracted = m[2 + 1] + (m[3 + 1] ? '#' + m[3 + 1] : '');
		if(islogpage)extracted = extracted.replace(RegExp("/[^#]*", 'g'),'');
...
Title.fromWikiText=function(txt, site) {
	return new Title().fromWikiText(txt, site);
};
Title.prototype.fromWikiText=function(txt, site) {
...
Title.prototype.userName=function(withNs) {
	var t=(new Title(this.value, this.site));
	t.toUserName(withNs);
	if (t.value) { return t; }
	return null;
};
Title.prototype.toTalkPage=function() {
	...
	{
		var namespaceIdArr = null;
		if(typeof this.site !== 'undefined'){if(this.site != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[this.site]) !== 'undefined')if(typeof pg.wiki.sitecatch[this.site].namespace !== 'undefined')
			namespaceIdArr = pg.wiki.sitecatch[this.site].namespace;
		};
		if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgFormattedNamespaces');
		var localizedNamespace = namespaceIdArr[namespaceId+1];
...
Title.prototype.namespace=function() {
		...
		if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgFormattedNamespaces');
	return namespaceIdArr[this.namespaceId()];
};
Title.prototype.namespaceId=function() {
	var n=this.value.indexOf(':');
	if (n<0) { return 0; } //mainspace
	var namespaceIdArr = null;
	if(typeof this.site !== 'undefined'){if(this.site != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[this.site]) !== 'undefined')if(typeof pg.wiki.sitecatch[this.site].namespaceid !== 'undefined')
		namespaceIdArr = pg.wiki.sitecatch[this.site].namespaceid;
	};
	if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgNamespaceIds');
	var namespaceId = namespaceIdArr[this.value.substring(0,n).split(' ').join('_').toLowerCase()];
...
Title.prototype.urlString=function(x) {
	...
	if(getSpecialRegexBySite(this.site).mypagereplace.test('/' + v)){
		var namespaceIdArr = null;
		if(typeof this.site !== 'undefined'){if(this.site != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[this.site]) !== 'undefined')if(typeof pg.wiki.sitecatch[this.site].namespace !== 'undefined')
			namespaceIdArr = pg.wiki.sitecatch[this.site].namespace;
		};
		if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgFormattedNamespaces');
		v = ('/' + v).replace(getSpecialRegexBySite(this.site).mypagereplace, 
			namespaceIdArr[pg.nsUserId] + ':' + mw.config.get("wgUserName") + "$3");
	}
	if (!x.omitAnchor && this.anchor) { v+= '#' + this.urlAnchor(); }
	if (!x.keepSpaces) { v=v.split(' ').join('_'); }
	if(this.isoldid) return encodeURI(v).split('?').join('%3F').split('+').join('%2B');
	return encodeURI(v).split('&').join('%26').split('?').join('%3F').split('+').join('%2B');
};
Title.prototype.removeAnchor=function() {
	return new Title(this.toString(true), this.site);
};
Title.prototype.toUrl=function() {
	if(this.checkWikia()){
		return "https://" + this.site + "/wiki/" + this.urlString() + "?";
	}
	return getIndexPath(this.site) + "?title=" + this.urlString();
	//return pg.wiki.titlebase + this.urlString();
};

function parseParams(url) {
	var get_site = RegExp("//([^/]+)").exec(url);
	var specialDiff;
	if(!!get_site && !!get_site[1]) specialDiff = getSpecialRegexBySite(get_site[1]).specialdiff.exec(url)
	else specialDiff = pg.re.specialdiff.exec(url);
	var is_history = false;
	
	if(!specialDiff){
		if(!!get_site && !!get_site[1]) specialDiff = getSpecialRegexBySite(get_site[1]).permanentlink.exec(url)
		else specialDiff = pg.re.permanentlink.exec(url);
		if(specialDiff)is_history = true;
	}
	
	if (specialDiff)
	{
		var split= specialDiff[1].split('/');
		if (split.length==1) return {oldid:split[0], diff: 'prev', not_diff: is_history};
		else if (split.length==2) return {oldid: split[0], diff: split[1], not_diff: is_history};
	}

	var ret={};
	if (is_history) {ret.not_diff = true;}
...
function isPopupLink(a) {
	...
	var h=a.href;
	h = h.replace(/Special:EntityPage\//g, '').replace(/Special%3AEntityPage%2F/g, '');
	if (h === document.location.href+'#') { return false; }
	if (!pg.re.basenames.test(h.replace(/^http:/, "https:"))) { return false; }
	if (!pg.re.urlNoPopup.test(h)) { return true;	}
	var get_site = RegExp("//([^/]+)").exec(h);
	var site_s = pg.wiki.sitebase;
	if(!!get_site){if(!!get_site[1]) site_s = get_site[1];}
	var site_r = getSpecialRegexBySite(site_s);
	
	return (
		(
			site_r.email.test(h) || site_r.contribs.test(h) || site_r.backlinks.test(h) || 
			site_r.specialdiff.test(h) || site_r.mypagereplace.test(h) || site_r.loglinks.test(h) || 
			site_r.permanentlink.test(h) || site_r.specialabusefilter.test(h)
		) &&
		h.indexOf('&limit=') == -1 );
}
...
function getPage(url, onComplete, owner) {
	...
	var callback= function (d) { 
		if (!d.aborted) {addPageToCache(d); onComplete(d);} 
	};
...
function Previewmaker(wikiText, baseUrl, owner) {
	...
	this.siteUrl=pg.wiki.articlebase;
	if(typeof owner === 'string'){
		this.siteUrl = getArticlePath(owner) + '/';
		this.owner=null;
	}else{	this.owner=owner;
		if(	this.owner){if(this.owner.article){
			this.siteUrl = getArticlePath(this.owner.article.site) + '/';
		}};
	}
...
Previewmaker.prototype.killComments = function () {
	...
	//
	this.data=this.data.replace(RegExp('\\[\\[\\/','g'), "[[" + this.owner.article.value + '/');
};
/**
   @private
*/
...
Previewmaker.prototype.killImages = function () {
	...
	var namespaceIdArr = null;
	if(typeof this.owner.article.site !== 'undefined'){if(this.owner.article.site != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[this.owner.article.site]) !== 'undefined')if(typeof pg.wiki.sitecatch[this.owner.article.site].namespaceid != 'undefined')
		namespaceIdArr = pg.wiki.sitecatch[this.owner.article.site].namespaceid;
	};
	if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgNamespaceIds');
	jQuery.each(namespaceIdArr, function(_localizedNamespaceLc, _namespaceId) {
...
Previewmaker.prototype.firstBit = function () {
	...
	var is_code = false;
	var is_wikidata = false;
	if(d.substring(0, 10) == "!!!code!!!"){
	...
	wikidata_head = '';
	if(d.substring(0, 16) == "!!!iswikidata!!!"){
		d = d.substring(16, d.length);
		d_num = d.indexOf("!!!subtail!!!==") + ("!!!subtail!!!==").length;
		wikidata_head = d.substring(0, d_num);
		d = d.substring(d_num, d.length);
		this.data = d;
		is_wikidata = true;
	}
	...
	dd = (wikidata_head + dd).replace(/!!!newline!!!/g, "\n<br/>").replace(/!!!anchorheadbegin!!!/g, "<a href=\"").replace(/!!!anchorheadend!!!/g, "\" >").replace(/!!!anchortail!!!/g, "</a>").
			replace(/!!!subhead!!!/g, "<sub>").replace(/!!!subtail!!!/g, "</sub>").
			replace(/!!!zh!!!/g, "https://zh.wikipedia.org/wiki/").replace(/!!!en!!!/g, "https://en.wikipedia.org/wiki/").replace(/!!!ja!!!/g, "https://ja.wikipedia.org/wiki/");
	if(is_code){
		dd = dd.replace(/(\/\/[^\n]*((?=\n)|$)|\/\*(\*(?!\/)|[^\*])+\*\/)/g, 
				function(match){
					return "<span class=\"c1\" style=\"color: #408080; font-style: italic;\">" + match.replace(/if/g, "i&#102;").replace(/for/g, "&#102;or")
						.replace(/switch/g, "&#115;witch").replace(/function/g, "&#102;unction").replace(/var/g, "&#118;ar")
						.replace(/return/g, "&#114;eturn").replace(/do/g, "&#100;o").replace(/while/g, "&#119;hile").replace(/in/g, "&#105;n")
						.replace(/else/g, "&#101;lse") + "</span>"
				}).
			replace(/(^|[ ;\(\)\{\}\[\]\t\n\r<>])(if|else|elseif|for|do|while|in|switch|case|function|return)(?=([ ;\(\{\[\t\n\r<>]|$))/g, 
				"$1<span class=\"kr\" style=\"color: #008000; font-weight: bold;\">$2</span>").
			replace(/(^|[ ;\)\}\]\t\n\r<>])(var)(?=([ \t\n\r]|$))/g, 
				"$1<span class=\"kr\" style=\"color: #008000; font-weight: bold;\">$2</span>").
			replace(/(&#39;(&(?!#39;)|[^&\\]|\\(&#39;|([^&]|&)))*&#39;|&#34;(&(?!#34;)|[^&\\]|\\(&#34;|([^&]|&)))*&#34;)/g, 
				"<span class=\"s2\" style=\"color: #BA2121;\">$1</span>")
			;
		dd = "<div class=\"mw-highlight\"><pre>" + dd + "</pre></div>";
	}
	this.data = dd;
};
/**
...
Previewmaker.prototype.makePreview = function() {
	...
	this.html=wiki2html(this.data, this.baseUrl, this.siteUrl); // needs livepreview
	this.fixHTML();
...
Previewmaker.prototype.editSummaryPreview=function() {
	...
	Insta.conf.paths.articles = this.siteUrl;
	var match;
...
function previewSteps(txt) {
	...
	var site_u = pg.wiki.articlebase;
	if(this.owner){if(this.owner.article){
		site_u = getArticlePath(this.owner.article.site) + '/';
	}};
	...
	p.html=wiki2html(p.data, base, site_u); // needs livepreview
...
function loadAPIArticleTitle(article, oldid, navpop) {
	var url = getApiPath(article.site) + '?format=json&formatversion=2&action=query&revids=' + oldid;
...
function loadAPIPreview(queryType, article, navpop) {
	var art=new Title(article, article.site).urlString();
	//var url=pg.wiki.apiwikibase + '?format=json&formatversion=2&action=query&';
	var url = getApiPath(article.site) + '?format=json&formatversion=2&action=query&';
	...
	case 'history':
		...
		break;
	case 'loglinks':
		url += 'list=logevents&format=json' + 
			((typeof article.byUser !== 'undefined') ? "&leuser=" + new Title(article.byUser, article.site).urlString() : '') +
			((typeof article.logtype !== 'undefined') ? "&letype=" + new Title(article.logtype, article.site).urlString() : '')
			+ (getSpecialRegexBySite(article.site).loglinks.test('/' + art) ? '' : ('&letitle=' + art));
		htmlGenerator=APILogEventPreviewHTML;
		break;
	case 'category':
		...
		break;
	case 'userinfo':
		var username = new Title( article, article.site ).userName();
		usernameart = encodeURIComponent( username );
		if(!article.checkWikia()){
			...
			url = 'https://' + article.site + '/api/v1/User/Details?ids=' + usernameart;
			htmlGenerator=APIWikiaUserInfoPreviewHTML;
		}
		break;
	case 'contribs':
		usernameart = encodeURIComponent( new Title( article, article.site ).userName() );
		url += 'list=usercontribs&ucuser=' + usernameart +
			'&uclimit=' + getValueOf('popupContribsPreviewLimit');
		htmlGenerator=APIcontribsPreviewHTML;
		break;
	case 'specialabusefilter':
		url += '&list=abusefilters&abfstartid=' + art + '&abfendid=' + art + '&abfprop=id|description|pattern|actions|hits|comments|lasteditor|lastedittime|status|private';
		art.isSpecialabusefilter = true;
		htmlGenerator=APIspecialabusefilterPreviewHTML;
		break;
	...
	case 'revision':
		...
		if(!article.checkWikia()){
			...
			url = 'https://' + article.site + '/api/v1/Articles/Details?ids=50&titles=' + art + '&abstract=100&width=200&height=200';
			htmlGenerator=APIrevisionWikiaPreviewHTML;
		}
		break;
	}
...
function linkList(list, article_site) {
	list.sort(function(x,y) { return (x==y ? 0 : (x<y ? -1 : 1)); });
	var buf=[];
	for (var i=0; i<list.length; ++i) {
		buf.push(wikiLink({article: new Title(list[i], article_site),
...
function editPreviewTable(article, h, reallyContribs, timeOffset) {
	...
	var curart=article;
	var page=null;
	var cur_titlebase = getIndexPath(curart.site) + "?title=";
	...
	if(reallyContribs) {
		...
		makeFirstColumnLinks = function(currentRevision) {
			var result = '(';
			result += '<a href="' + cur_titlebase +
				new Title(currentRevision.title, curart.site).urlString() + '&diff=prev' +
				'&oldid=' + currentRevision.revid + '">' + popupString('diff') + '</a>';
			result += '&nbsp;|&nbsp;';
			result += '<a href="' + cur_titlebase + 
				new Title(currentRevision.title, curart.site).urlString() + '&action=history">' +
				popupString('hist') + '</a>';
			result += ')';
			return result;
		};
		...
		makeFirstColumnLinks = function(currentRevision) {
			var result = '(';
			result += '<a href="' + cur_titlebase + new Title(curart, curart.site).urlString() +
	...
	for (var i=0; i<h.length; ++i) {
		if (reallyContribs) { 
			page = h[i].title;
			curart = new Title(page, curart.site);
		}
		...
		html.push('<td>' +
			'<a href="' + cur_titlebase + new Title(curart, curart.site).urlString() +
		...
		if (!reallyContribs) {
			...
			if( !h[i].userhidden ) {
				if( pg.re.ipUser.test(user) ) {
					col3url=cur_titlebase + mw.config.get('wgFormattedNamespaces')[pg.nsSpecialId] + ':Contributions&target=' + new Title(user, curart.site).urlString();
				} else {
					col3url=cur_titlebase + mw.config.get('wgFormattedNamespaces')[pg.nsUserId] + ':' + new Title(user, curart.site).urlString();
				}
			...
			col3url=cur_titlebase + curart.urlString();
		...
		if (c) {
			comment=new Previewmaker(c, new Title(curart, curart.site).toUrl(), curart.site).editSummaryPreview();
...
function showAPIPreview(queryType, html, id, navpop, download) {
	...
	switch (queryType) {
		...
		case 'wikiatext':
			insertWikiaPreview(download, html); return;
		case 'revision':
			if(!download.missing) insertPreview(download);
			else{ 
				if(navpop.article.checkWikia()) setPopupHTML("", "popupPreview", navpop.idNumber);
				else setPopupHTML(popupString('PageDoesntExist'), "popupPreview", navpop.idNumber);
			}
			return;
	}
	setPopupTipsAndHTML(html, target, id);
}
function FlowPreviewHTML(article, download) {
	try{
		var cur_apiurl = getApiPath(article.site) + '?action=flow&submodule=view-topiclist&format=json&page=';

		var get_flowdata = function(){
			getPageWithCaching(cur_apiurl + article.urlString(), function(data){
				if(data){if(data.data){var jsonstr = data.data;try
				{
					var jsObj=getJsObj(jsonstr).flow["view-topiclist"];

					if(jsObj.status === 'ok' && jsObj.result.topiclist){
						showAPIPreview('flowdata', FlowTalkTable(article, jsObj.result.topiclist, getTimeOffset()), download.owner.idNumber, download.owner, data);
					
					}
					
				} catch(flowError) {}}}
				
				//APILogEventPreviewHTML(article, data, download.owner), download.owner.idNumber, download.owner, data);
			}, download.owner);
			return true;
		};
		if (download.owner.visible || !getValueOf('popupLazyDownloads')) { 
			get_flowdata(); 
		}
		else { 
			download.owner.addHook(get_flowdata, 'unhide', 'before', 'DOWNLOAD_flowdata_QUERY_DATA'); 
		}
		return popupString("FlowPage");
	} catch(someError) {
		return popupString('MessageFlowPageError');
	}
}
function FlowTalkTable(article, h, timeOffset) {
	if(!h)return;
	var html=['<table>'];
	var day=null;
	var curart=article;
	var page=null;
	var topic_inedx = h.roots;
	var topic_hash = h.posts;
	var topic_data = h.revisions;

	var cur_titlebase = getIndexPath(curart.site) + "?title=";
	var max_show = 20;
	for (var i=0; i<topic_inedx.length && i<max_show; ++i) {
		var topic_it = topic_data[topic_hash[topic_inedx[i]]];
		var editDate = adjustDate(getDateFromTimestamp(
			topic_it.timestamp.replace(/^([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})$/, "$1-$2-$3-$4-$5-$6")), timeOffset);
		var thisDay = dayFormat(editDate);
		var thisTime = timeFormat(editDate);
		if (thisDay == day) {
			thisDay = '';
		...
		html.push('<tr class="popup_history_row_' + ( (i%2) ? 'odd' : 'even') + '">');
		html.push('<td>' + topic_it.content.content + '</td>');
		html.push('<td><a href="' + cur_titlebase + topic_it.articleTitle + '">' + thisTime + '</a></td>');
		html.push('<td><a href="' + topic_it.creator.links.userpage.url + '">' + topic_it.creator.links.userpage.title + '</a></td>');
		html.push('</tr>');
		html=[html.join('')];
	}
	html.push('</table>');
...
function APIrevisionPreviewHTML(article, download) {
	try{
		...
		if(article.value === '' || article.value === null || (typeof article.value === 'undefined')){
			article.value = page.title;
			download.missing_title = article;
		}
		if( page.missing ) {
			...
			var cur_apiurl = getApiPath(article.site) + '?action=query&list=logevents&format=json&letitle=';
			var get_logevent = function(){
				getPageWithCaching(cur_apiurl + article.urlString(), function(data){
					download.missing = false;
					showAPIPreview('logevent', APILogEventPreviewHTML(article, data, download.owner, popupString('PageDoesntExist'), 
						popupString('PageDoesntExist') + popupString('MessageLogRef') + "<br/>"), download.owner.idNumber, download.owner, data);
				}, download.owner);
				return true;
			};
			if (download.owner.visible || !getValueOf('popupLazyDownloads')) { 
				...
				download.owner.addHook(get_logevent, 'unhide', 'before', 'DOWNLOAD_logevent_QUERY_DATA'); 
			}
			return popupString('PageDoesntExist');
		}
		var content = (
			page &&
			page.revisions &&
...
function APIrevisionWikiaPreviewHTML(article, download) {
	try{
		var jsObj=getJsObj(download.data);
		var page=anyChild(jsObj.items);
			download.missing = true;
			var cur_apiurl = 'https://' + article.wikia  + '.wikia.com/api/v1/Articles/AsSimpleJson?id=' + page.id;
			var get_wikiatext = function(){
				getPageWithCaching(cur_apiurl, function(data){
					download.missing = false;
					showAPIPreview('wikiatext', page, download.owner.idNumber, download.owner, data);
				}, download.owner);
				return true;
			};
			if (download.owner.visible || !getValueOf('popupLazyDownloads')) { 
				...
				download.owner.addHook(get_wikiatext, 'unhide', 'before', 'DOWNLOAD_wikiatext_QUERY_DATA'); 
			}
			return popupString('PageDoesntExist');
	} catch(someError) {
		return 'Revision preview failed :(';
...
function APIbacklinksPreviewHTML(article, download/*, navpop*/ ) {
	try {
		...
		for ( var i=0; i < list.length; i++ ) {
			var t=new Title(list[i].title, article.site);
			//var cur_titlebase=pg.wiki.titlebase;

			var cur_titlebase=getIndexPath(article.site) + "?title=";
			if(article.checkWikia()){
				cur_titlebase = "https://" + article.site + "/wiki/" + article.urlString();
			}
			html.push('<a href="' + cur_titlebase + t.urlString() + '">' + t + '</a>');
		}
		html=html.join(', ');
...
function APIimagepagePreviewHTML(article, download, navpop) {
	try {
		...
		if (page && page.imagerepository == "shared" ) {
			var art=new Title(article, article.site);
...
function APIimagelinksPreviewHTML(article, download) {
	try {
		...
		if (list) {
			...
			return '<h2>' + popupString('File links') + '</h2>' + linkList(ret, article.site);
		} else {
...
function APIcategoryPreviewHTML(article, download) {
	try{
		...
		ret = '<h2>' + tprintf('Category members (%s shown)', [ret.length]) + '</h2>' +linkList(ret, article.site);
...
function APIspecialabusefilterPreviewHTML(article, download) {
	try{
		var jsObj=getJsObj(download.data);
		var abusefilter=anyChild(jsObj.query.abusefilters);
		var ret='';
		var abusefilter_pattern = new Previewmaker((abusefilter.pattern ? "!!!code!!!" + abusefilter.pattern : "(過濾器不公開)").replace(/[ ]/g, "&nbsp;").replace(/[<]/g, "&lt;"), '', '');
		abusefilter_pattern.firstBit();
		abusefilter_pattern.killBadWhitespace();
		ret = abusefilter_pattern.data || (abusefilter.pattern ? abusefilter.pattern : "(過濾器不公開)");
		ret = ret.replace(/[\n\r]/g, '');
		var popup_title = anyChild(document.getElementById("popupTopLinks" + download.owner.idNumber).children);
		var popup_description = document.getElementById("popupData" + download.owner.idNumber);
		if(popup_title){
			var popup_title_main = popup_title.firstChild;
			var href = popup_title_main.getAttribute("href");
			if(href) popup_title_main.setAttribute("href", ('' + href).replace(/(\?title=)/g, "$1Special:AbuseFilter/"));
			popup_title_main.innerHTML = "防濫用過濾器" + popup_title_main.innerHTML;
		}
		if(popup_description){
			popup_description.innerHTML = abusefilter['description'] //關鍵字衝突
				+ ( abusefilter.hits ? (", 觸發" + abusefilter.hits + "次") : '');
			
		}
		return "<hr/>" + ret;
	} catch(someError) {
		return 'Abuse filter preview failed :(';
	}
}

function APIWikiaUserInfoPreviewHTML(article, download) {
	var ret=[];
	var queryobj = {};
	try{
...
function APIhistoryPreviewHTML(article, download, navpop, reallyContribs) {
	try {
		...
		return popupString('MessageHistoryError');
	}
}

function APILogEventPreviewHTML(article, download, navpop, message, message_have_data) {
	var prefix_msg = ((typeof message !== 'undefined') ? message : popupString('MessageNoLog'));
	var postfix_msg = ((typeof message_have_data !== 'undefined') ? message_have_data : '');
	try {
		var jsobj=getJsObj(download.data);
		var logevents = [];
		logevents=jsobj.query.logevents;
		var ret=LogEventTable(article, logevents, getTimeOffset());
		if (logevents.length > 0)ret = postfix_msg + ret;
		else ret = prefix_msg;
		return ret;
	} catch (someError) {
		return popupString('MessageLogEventError');
	}
}

function getSiteOrData(input, token, meta, default_val){
	var check = token + ((meta != '') ? '_' : '') + meta + '_' + input;
	var result = popupString(check);
	if (result==check) return default_val;
	else return result;
}

function LogEventTable(article, h, timeOffset) {
	var html=['<table>'];
	var day=null;
	var curart=article;
	var page=null;
	...
	for (var i=0; i<h.length; ++i) {
		...
		html.push('<tr class="popup_history_row_' + ( (i%2) ? 'odd' : 'even') + '">');
		html.push('<td><a href="' + cur_titlebase + "Special:Log/" + h[i].type + "&page=" + 
			new Title(curart, curart.site).urlString() + '">' + getSiteOrData(h[i].type, "logevent", "type", h[i].type) + '</a></td>');
		html.push('<td>' + thisTime + '</td>');
		var col3url='', col3txt='';
		var user=h[i].user;
		if( !h[i].userhidden ) {
			if( pg.re.ipUser.test(user) ) {
		...
		html.push('<td><a href="' + col3url + '">' + col3txt + '</a></td>');
		
		var LoadEventDataString = ((typeof window.a2569875PopUp.Module.generationLogEventString === 'function') ? window.a2569875PopUp.Module.generationLogEventString : function(data){return null})
			(h[i]);
		LoadEventDataString = ((LoadEventDataString !== null && LoadEventDataString !== '' && (typeof LoadEventDataString !== 'undefined')) ? 
			new Previewmaker(LoadEventDataString, new Title(curart, curart.site).toUrl(), curart.site).editSummaryPreview() :
			getSiteOrData(h[i].action, "logevent", '', h[i].action))
		
		html.push('<td>' + LoadEventDataString + '</td>');
...
function loadImage(image, navpop) {
	...
	//var url=pg.wiki.apiwikibase + '?format=json&formatversion=2&action=query';
	var url = getApiPath(navpop.article.site) + '?format=json&formatversion=2&action=query';
	...
	if(image.substring(0, 4) == "http"){
		var popupImage = document.getElementById("popupImg" + navpop.idNumber);
		if (!popupImage) {
			log( "could not find insertion point for image");
			return;
		}
		...
		var popupImage_a=document.getElementById("popupImageLink" + navpop.idNumber);
		popupImage_a.onclick=toggleSize;
		popupImage_a.title=popupString('Toggle image size');
		return;
	}
...
function getValidImageFromWikiText(wikiText, wikisite) {
	...
	if (!matched) { 
		var out_side_image=RegExp("((https?)?//.+\.(png|gif|bmp|jpe?g))");
		var match_oside_img = out_side_image.exec(t);
		var other_imglink = null;
		if(match_oside_img != null){
			if(match_oside_img[0] != null){
				other_imglink = match_oside_img[0];
				if(match_oside_img[0].indexOf("http") < 0){
					other_imglink = "https:" + other_imglink;
				}
				return other_imglink;
			}
		}
		return null; 
	}
...
function nsRe(namespaceId, wikisite) {
	var imageNamespaceVariants = [];
	var namespaceIdArr = null;
	if(typeof wikisite !== 'undefined'){if(wikisite != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[wikisite]) !== 'undefined')if(typeof pg.wiki.sitecatch[wikisite].namespaceid !== 'undefined')
		namespaceIdArr = pg.wiki.sitecatch[wikisite].namespaceid;
	};
	if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgNamespaceIds');
	jQuery.each(namespaceIdArr, function(_localizedNamespaceLc, _namespaceId) {
		if (_namespaceId!=namespaceId) return;
		_localizedNamespaceLc = upcaseFirst(_localizedNamespaceLc);
		imageNamespaceVariants.push(mwRegExpescape(_localizedNamespaceLc).split(' ').join('[ _]'));
		imageNamespaceVariants.push(mwRegExpescape(encodeURI(_localizedNamespaceLc)));
	});
	return '(?:' + imageNamespaceVariants.join('|') + ')';
}

function nsReImage(wikisite) {
	return nsRe(pg.nsImageId, wikisite);
}
...
function doSelectionPopup() {
	...
	a.href=getIndexPath(article.site) + "?title=" + article;
	if(article.checkWikia()){
		a.href = "https://" + article.site + "/wiki/" + article.urlString();
	}
...
function setSiteInfo() {
	...
	pg.wiki.sitecatch = {};
	pg.wiki.sitecatch[pg.wiki.sitebase] = {};
	pg.wiki.sitecatch[pg.wiki.sitebase].namespace = {};
	pg.wiki.sitecatch[pg.wiki.sitebase].namespaceid = {};
	pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases = {};
	pg.wiki.sitecatch[pg.wiki.sitebase].specialPageName = {};
	//specialPageAliases
	...
	pg.wiki.apibaseData = [ 
		...
		{ titletail:"/api.php", sites:[
			"gamepedia.com", "moegirl.org", "micdoodle8.com", "komica.org", "uncyclopedia.tw", "uncyclopedia.hk", "ncyclopedia.info"
		]} 
	];
	pg.wiki.articlebaseData = [ 
		{ titletail:"/wiki", sites:[
			"wikidata.org", "wikipedia.org", "wikimedia.org", "wikisource.org", "wikiquote.org", "wikibooks.org", "wikinews.org",
			"wikivoyage.org", "wikiversity.org", "wiktionary.org", "mediawiki.org", "minecraftwiki.net", "uncyclopedia.tw", "uncyclopedia.hk",
			"wikia.com", "uncyclopedia.co"
		]}, 
		{ titletail:"", sites:[
			"gamepedia.com", "moegirl.org", "micdoodle8.com", "komica.org"
		]} 
	];
	pg.wiki.titlebaseData = [ 
		{ titletail:"/w/index.php?title=", sites:[
			"wikidata.org", "wikipedia.org", "wikimedia.org", "wikisource.org", "wikiquote.org", "wikibooks.org", "wikinews.org",
			"wikivoyage.org", "wikiversity.org", "wiktionary.org", "mediawiki.org", "minecraftwiki.net", "uncyclopedia.co"
		]}, 
		{ titletail:"/index.php?title=", sites:[
			"gamepedia.com", "moegirl.org", "micdoodle8.com", "komica.org", "uncyclopedia.tw", "uncyclopedia.hk"
		]} 
	];
}
function getSpecialRegexBySite(siteBase){
	if(!!pg.wiki.sitecatch[siteBase]){
		if(!!pg.wiki.sitecatch[siteBase].specialRegexp) return pg.wiki.sitecatch[siteBase].specialRegexp;
	}
	return pg.re;
}
function setSpecialPageBase(specialList, query, siteBase) {
	for(i in specialList) {
		if(typeof specialList[i] !== 'string') continue;
		for(j in query) {
			if(!query[j].realname) continue;
			if(query[j].realname.toLowerCase() == specialList[i]){
				pg.wiki.sitecatch[siteBase].specialPageAliases[specialList[i]] = { list: query[j].aliases };
				break;
			}
		} 
		var regex = ''; var first_flag = true;
		for(j in pg.wiki.sitecatch[siteBase].specialPageAliases[specialList[i]].list) { 
			if(!pg.wiki.sitecatch[siteBase].specialPageAliases[specialList[i]]) continue;
			var alia = pg.wiki.sitecatch[siteBase].specialPageAliases[specialList[i]].list[j];
			if(typeof alia !== 'string') continue;
			if(first_flag) { first_flag = false } else{
				regex += '|';
			}
			regex += literalizeRegex(encodeURI(alia)) + '|' + literalizeRegex(encodeURI(alia.toLowerCase())) + '|' + 
				literalizeRegex(alia) + '|' + literalizeRegex(alia.toLowerCase()) + '|' + 
				literalizeRegex(alia.substring(0, 1).toUpperCase() + alia.substring(1).toLowerCase());
			pg.wiki.sitecatch[siteBase].specialPageName[alia.toLowerCase()] = specialList[i];
		}
		pg.wiki.sitecatch[siteBase].specialPageAliases[specialList[i]].regex = regex;
	} 
	pg.wiki.sitecatch[siteBase].specialRegexp = {};
	
	var sp=nsRe(pg.nsSpecialId, siteBase);
	pg.wiki.sitecatch[siteBase].specialRegexp.contribs  =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['contributions'].regex +
		')' + '(&target=|/|/' + nsRe(pg.nsUserId, siteBase)+':)(.*)') ;
	pg.wiki.sitecatch[siteBase].specialRegexp.email	    =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['emailuser'].regex +
		')'	 + '(&target=|/|/(?:' + nsRe(pg.nsUserId, siteBase)+':)?)(.*)') ;
	pg.wiki.sitecatch[siteBase].specialRegexp.backlinks =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['whatlinkshere'].regex +
		')' + '(&target=|/)([^&]*)');
	pg.wiki.sitecatch[siteBase].specialRegexp.specialdiff=RegExp('/' + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['diff'].regex +
		')/([^?#]*)');
	pg.wiki.sitecatch[siteBase].specialRegexp.mypagereplace=RegExp('(title=|/)(' + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['mypage'].regex +
		'))(/[^?#]*)?');
	pg.wiki.sitecatch[siteBase].specialRegexp.loglinks =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['log'].regex +
		')(?:&(?!page)(?!user)[^&]*)*((&user=|\/)([^&]*))?((&(?!page)[^&]*)*(&page=)([^&]*)(&(?!user)[^&]*)*((&user=)([^&]*))?)?');
	pg.wiki.sitecatch[siteBase].specialRegexp.permanentlink =RegExp('/' + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['permanentlink'].regex +
		')/([^?#]*)');
	pg.wiki.sitecatch[siteBase].specialRegexp.specialabusefilter=RegExp('(title=|/)' + sp + '(?:%3A|:)(?:'
			+ pg.wiki.sitecatch[siteBase].specialPageAliases['abusefilter'].regex +
		')/(?:history/)?([^?#A-Za-z/]+)');
}
function setTitleBase() {
	...
	var titlebaseArray = [ protocol + '//' + pg.wiki.sitebase + titletail ];
	pg.wiki.titlebase   = protocol + '//' + pg.wiki.sitebase + titletail;

	pg.wiki.titlebaseData.forEach(function(element) {
		element.sites.forEach(function(site_name) {
			titlebaseArray.splice(-1, 0, protocol + '//' + "_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + site_name + element.titletail);
		});
	});
	if(window.otherbaseData) window.otherbaseData.forEach(function(element) {
		element.sites.forEach(function(site_name) {
			element.pagetail.forEach(function(pagetail) {
				titlebaseArray.splice(-1, 0, protocol + '//' + "_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + site_name + pagetail);
			});
		});
	});

	pg.wiki.titlebasetail  = titletail;
	pg.wiki.titlebasehead  = protocol + '//';
	pg.wiki.apibasetail = pg.wiki.APIPath;
	...
	var articlebaseArray = [ protocol + '//' + pg.wiki.sitebase + pg.wiki.articlePath ];
	pg.wiki.articlebase = protocol + '//' + pg.wiki.sitebase + pg.wiki.articlePath;

	pg.wiki.articlebaseData.forEach(function(element) {
		element.sites.forEach(function(site_name) {
			articlebaseArray.splice(-1, 0, protocol + '//' + "_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + site_name + element.titletail);
		});
	});
	...
	var pre_regex = map( literalizeRegex, titlebaseArray.concat(articlebaseArray)).join('|');
	pre_regex = pre_regex.replace(/_left/g, "[").replace(/_right/g, "]").replace(/_desh/g, "-").replace(/_plus/g, "+").replace(/_questionmark/g, "?").replace(/_slash/g, "|").replace(/_cleft/g, "(").replace(/_cright/g, ")").replace(/_colon/g, ":");
	pg.re.basenames = RegExp( '^(' + pre_regex + ')' );
}
...
function setMainRegex() {
	...
	var reEnd='(' + preTitles + ')([^&?#]*)[^#]*(?:#(.+))?';
	var sitesArray = [ literalizeRegex(pg.wiki.sitebase) ];
	pg.wiki.articlebaseData.forEach(function(element) {
		element.sites.forEach(function(site_name) {
			sitesArray.splice(-1, 0, literalizeRegex("_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + site_name));
		});
	});
	if(window.otherbaseData) window.otherbaseData.forEach(function(element) {
		element.sites.forEach(function(site_name) {
	...
	var pre_regex = '(' +sitesArray.join('|') + ')';
	pre_regex = pre_regex.replace(/_left/g, "[").replace(/_right/g, "]").replace(/_desh/g, "-").replace(/_plus/g, "+").replace(/_questionmark/g, "?").replace(/_slash/g, "|").replace(/_cleft/g, "(").replace(/_cright/g, ")").replace(/_colon/g, ":");
	pg.re.specialwikisitelink = RegExp("([a-z\\-]+\\.gamepedia\\.com|[a-z]+\\.moegirl\\.org|wiki\\.micdoodle8\\.com)");
	pg.re.main = RegExp(reStart + pre_regex + reEnd);
}

function setRegexps() {
	setMainRegex();
	var sp=nsRe(pg.nsSpecialId, pg.wiki.sitebase);
	pg.re.urlNoPopup=RegExp('((title=|/)' + sp + '(?:%3A|:)|section=[0-9]|^#$)') ;
	
	if(!pg.re.specialDone){
...
//</NOLITE>
	...
	for(var it in pg.wiki.apibaseData){ 
		if(typeof pg.wiki.apibaseData[it].sites === 'undefined') continue;
		pg.wiki.apibaseData[it].regex = RegExp('(' + literalizeRegex("_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + pg.wiki.apibaseData[it].sites.join("_slash_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark")).replace(/_left/g, "[").replace(/_right/g, "]").replace(/_desh/g, "-").replace(/_plus/g, "+").replace(/_questionmark/g, "?").replace(/_slash/g, "|").replace(/_cleft/g, "(").replace(/_cright/g, ")").replace(/_colon/g, ":") + ')');
	}
	for(var it in pg.wiki.titlebaseData){ 
		if(typeof pg.wiki.titlebaseData[it].sites === 'undefined') continue;
		pg.wiki.titlebaseData[it].regex = RegExp('(' + literalizeRegex("_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + pg.wiki.titlebaseData[it].sites.join("_slash_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark")).replace(/_left/g, "[").replace(/_right/g, "]").replace(/_desh/g, "-").replace(/_plus/g, "+").replace(/_questionmark/g, "?").replace(/_slash/g, "|").replace(/_cleft/g, "(").replace(/_cright/g, ")").replace(/_colon/g, ":") + ')');
	}
	for(var it in pg.wiki.articlebaseData){ 
		if(typeof pg.wiki.articlebaseData[it].sites === 'undefined') continue;
		pg.wiki.articlebaseData[it].regex = RegExp('(' + literalizeRegex("_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark" + pg.wiki.articlebaseData[it].sites.join("_slash_cleft_questionmark_colon_lefta_deshz-._right_plus._cright_questionmark")).replace(/_left/g, "[").replace(/_right/g, "]").replace(/_desh/g, "-").replace(/_plus/g, "+").replace(/_questionmark/g, "?").replace(/_slash/g, "|").replace(/_cleft/g, "(").replace(/_cright/g, ")").replace(/_colon/g, ":") + ')');
	}
}
...
function setThisSiteName(){
	var lang = null;
	var site_type = null;
	var base = getArticlePath(pg.wiki.sitebase) + '/'
	if(RegExp("[a-zA-Z0-9\\-\\.]+\\.wikia\\.com").test(pg.wiki.sitebase)){
		var wikiasite_test = RegExp("([^\n]+)\\.wikia\\.com").exec(pg.wiki.sitebase);
		var lang = wikiasite_test[1];
		var site_type = "wikia";
	}else{
		var gamepedia_test = RegExp("([a-z]+)(\\-)?([a-z]+)?\\.gamepedia");
		var language_test = RegExp("([a-z\-]+)\\.(moegirl|(wik(ipedia|isource|iquote|ibooks|inews|ivoyage|iversity|tionary)))");
		var wikimedia_test = RegExp("([a-z\-]+)\\.wikimedia");
		var other_test = RegExp("[a-z\-]+\\.([a-z\-]+)\\.");
		var gamepedia_result = gamepedia_test.exec(pg.wiki.sitebase);
		var language_result = language_test.exec(pg.wiki.sitebase);
		var wikimedia_result = wikimedia_test.exec(pg.wiki.sitebase);
		var other_result = other_test.exec(pg.wiki.sitebase);
		if(pg.wiki.sitebase == "uncyclopedia.tw"){
			lang = "zh-tw";
			site_type = "uncyclopedia";
		}else if (gamepedia_result){
			lang = gamepedia_result[3];
			site_type = gamepedia_result[1];
			if(!lang)lang = "en";
		}else if(language_result){
			lang = language_result[1];
			site_type = language_result[2];
		}else if(wikimedia_result){
			site_type = wikimedia_result[1];
		}else if(other_result){
			site_type = other_result[1];
		}
	}
	pg.wiki.siteSummary = function(){ 
		return popupString('SummaryByPopupWithoutPopupName') + "[[:w:zh:User:A2569875-sandbox/MyPopups|a2569875's popup Mod]] @" + getSiteOrData("addr", site_type, '', site_type) + ( (!lang) ? '' : ':' + lang );
	};
	window.console.log(pg.wiki.siteSummary());
}
...
function setupPopups( callback ) {
	...
	pg.wiki.specialNeed = [ 'contributions', 'emailuser', 'whatlinkshere', 'diff', 'mypage', 'log', 'permanentlink', 'abusefilter' ];
	// These dependencies are also enforced from the gadget,
	// but not everyone loads this as a gadget, so double check
	var doing_loadwikiapi = function() {
		if(mw.util.escapeRegExp)mwRegExpescape=mw.util.escapeRegExp;
		else mwRegExpescape=mw.RegExp.escape;
		...
		mw.loader.using( 'mediawiki.api' ).done( function () {
			var api = new mw.Api();
			    api.get( {
			        ...
			        for(i in data.query['namespaces']){
			        	if(data.query['namespaces'][i]){
				        	pg.wiki.sitecatch[pg.wiki.sitebase].namespace[i] = data.query['namespaces'][i].name;
				        	pg.wiki.sitecatch[pg.wiki.sitebase].namespaceid[data.query['namespaces'][i].name.split(' ').join('_').toLowerCase()] = parseInt(i);
			        	}
			        }
			        for(i in data.query.namespacealiases){
			        	if(data.query.namespacealiases[i] && data.query.namespacealiases[i].alias)
			        		pg.wiki.sitecatch[pg.wiki.sitebase].namespaceid[data.query.namespacealiases[i].alias.split(' ').join('_').toLowerCase()] = parseInt(data.query.namespacealiases[i].id);
			        }
			        setSpecialPageBase(pg.wiki.specialNeed, data.query['specialpagealiases'], pg.wiki.sitebase);
			    	pg.re.specialDone = true;
					var sp=nsRe(pg.nsSpecialId, pg.wiki.sitebase);
					pg.re.contribs  =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['contributions'].regex +
						')' + '(&target=|/|/' + nsRe(pg.nsUserId, pg.wiki.sitebase)+':)(.*)') ;
					pg.re.email	    =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['emailuser'].regex +
						')'	 + '(&target=|/|/(?:' + nsRe(pg.nsUserId, pg.wiki.sitebase)+':)?)(.*)') ;
					pg.re.backlinks =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['whatlinkshere'].regex +
						')' + '(&target=|/)([^&]*)');
					pg.re.specialdiff=RegExp('/' + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['diff'].regex +
						')/([^?#]*)');
					pg.re.mypagereplace=RegExp('(title=|/)(' + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['mypage'].regex +
						'))(/[^?#]*)?');
					pg.re.loglinks =RegExp('(title=|/)'  + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['log'].regex +
						')(?:&(?!page)(?!user)[^&]*)*((&user=|\/)([^&]*))?((&(?!page)[^&]*)*(&page=)([^&]*)(&(?!user)[^&]*)*((&user=)([^&]*))?)?');
					pg.re.permanentlink =RegExp('/' + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['permanentlink'].regex +
						')/([^?#]*)');
					pg.re.specialabusefilter=RegExp('(title=|/)' + sp + '(?:%3A|:)(?:'
							+ pg.wiki.sitecatch[pg.wiki.sitebase].specialPageAliases['abusefilter'].regex +
						')/(?:history/)?([^?#A-Za-z/]+)');
	...
	var complete_loadwikiapi = function(){mw.loader.using( ['mediawiki.util', 'mediawiki.user', 'user.options'] ).then(doing_loadwikiapi);};
	mw.loader.using( ['mediawiki.RegExp'],complete_loadwikiapi,complete_loadwikiapi);
}
// ENDFILE: init.js
...
function defaultNavlinkSpec() {
	...
	// misc links
	str += '<br><<whatLinksHere|shortcut=l>>*<<relatedChanges|shortcut=r>>*<<move|shortcut=m>>*<<purgeLink>>*<<logLink>>';
...
function expandConditionalNavlinkString(s,article,z,recursionCount) {
	...
	for (var i=1; i<splitted.length; i=i+numParens+1) {
		...
		case 'user':
			...
			break;
		case 'talk':
			if(!article.iswikia && !article.isSpecialabusefilter)testResult=(article.talkPage())?false:true; // talkPage converts _articles_ to talkPages
...
navlinkTag.prototype.getPrintFunction=function() { //think about this some more
	...
	this.noPopup=1;
	switch (this.id) {
	case 'contribs': case 'whatLinksHere': case 'logLink':
	case 'userPage': case 'monobook': case 'userTalk':
	case 'talk': case 'article': case 'lastEdit':
		this.noPopup=null;
		break;
	case 'history': case 'userlog': 
		if(!this.article.checkWikia())this.noPopup=null;
	}
	switch (this.id) {
	...
	case 'afhistory': 
		this.print=specialLink; this.specialpage='AbuseFilter'; this.sep='/history/'; break;
	case 'case' : 
		this.print=wikiLink; this.action=this.id; break;
	case 'delete':
...
function loadDiff(article, oldid, diff, navpop) {
	...
	//mediawiki.ForeignApi mw.ForeignApi
	//mediawiki.api

	var api_url = getApiPath(article.site);

	var apitype = 'mediawiki.api';
	if(article.site != pg.wiki.sitebase) apitype = 'mediawiki.ForeignApi';

	mw.loader.using( apitype ).then( function() {
		var api = null;
		if(article.site != pg.wiki.sitebase)
			api = new mw.ForeignApi( api_url, {
			    ajax: {
		...
		switch (diff) {
			...
			case 'prev':
				if( oldid ) {
					if(oldid != "cur") params.fromrev = oldid;
					else{
						params.fromtitle = article.value;
					}
				} else {
					params.fromtitle;
				}
		...
		then_func = function( data ) {
			...
			var go = function() {
				pendingNavpopTask(navpop);
				var url=api_url + '?format=json&formatversion=2&action=query&';
		...
		if(article.site != pg.wiki.sitebase) api.get( params ).then( then_func ).fail(on_post_err);
...
function revertSummary(oldid, diff) {
	...
	return ret + pg.wiki.siteSummary() + '&autorv=' + oldid;
}

function wikiSiteLink(l) {
	...
	var lang = null;
	var site_type = null;
	var base = getArticlePath(l.article.site) + '/'
	if(RegExp("[a-zA-Z0-9\\-\\.]+\\.wikia\\.com").test(l.article.site)){
		var wikiasite_test = RegExp("([^\n]+)\\.wikia\\.com").exec(l.article.site);
		var lang = wikiasite_test[1];
		var site_type = "wikia";
	}else{
		var gamepedia_test = RegExp("([a-z]+)(\\-)?([a-z]+)?\\.gamepedia");
		...
		var wikimedia_test = RegExp("([a-z\-]+)\\.wikimedia");
		var other_test = RegExp("[a-z\-]+\\.([a-z\-]+)\\.");
		var gamepedia_result = gamepedia_test.exec(l.article.site);
		var language_result = language_test.exec(l.article.site);
		var wikimedia_result = wikimedia_test.exec(l.article.site);
		var other_result = other_test.exec(l.article.site);
		if(l.article.site == "uncyclopedia.tw"){
			lang = "zh-tw";
			site_type = "uncyclopedia";
		}else if (gamepedia_result){
			lang = gamepedia_result[3];
	...
	return generalNavLink({url: base, newWin: l.newWin,
				title: l.article.site,
				text: ( (!site_type) ? popupString('OtherWikiSite') : getSiteOrData("addr", site_type, '', site_type) ) + ( (!lang) ? '' : ':' + lang ),
...
function titledWikiLink(l) {
	...
	var base = getIndexPath(l.article.site) + "?title=" + l.article.urlString();
	if(l.article.checkWikia()){
		base = "https://" + l.article.site + "/wiki/" + l.article.urlString() + "?";
	}
	var url=base;
...
function magicWatchLink(l) {
	...
	l.onclick = simplePrintf( 'pg.fn.modifyWatchlist(\'%s\',\'%s\',\'%s\');return false;', [l.article.toString(true).split("\\").join("\\\\").split("'").join("\\'"), this.id, l.article.site] );
	return wikiLink(l);
}

pg.fn.modifyWatchlist = function modifyWatchlist(title, action, site) {

	var api_url = getApiPath(site);

	var apitype = 'mediawiki.api';
	if(site != pg.wiki.sitebase) apitype = 'mediawiki.ForeignApi';
	//support for other wiki
	mw.loader.using( apitype ).then( function() {
		var api = null;
		if(site != pg.wiki.sitebase)
			api = new mw.ForeignApi( api_url, {
			    ajax: {
			        headers: { 'Api-User-Agent': pg.misc.userAgent }
			    }
...
function specialLink(l) {
	...
	var namespaceIdArr = null;
	if(typeof l.article.site !== 'undefined'){if(l.article.site != pg.wiki.sitebase && (typeof pg.wiki.sitecatch[l.article.site] !== 'undefined'))if(typeof pg.wiki.sitecatch[l.article.site].namespace !== 'undefined')
		namespaceIdArr = pg.wiki.sitecatch[l.article.site].namespace;
	};
	if(namespaceIdArr == null)namespaceIdArr = mw.config.get('wgFormattedNamespaces');

	var cur_titlebase = getIndexPath(l.article.site) + "?title=";

	var base = cur_titlebase +  namespaceIdArr[pg.nsSpecialId]+':'+l.specialpage;
...
function changeLinkTargetLink(x) { // newTarget, text, hint, summary, clickButton, minor, title (optional) {
	...
	// <nowiki>eg [[category:X1]] and [[Category:X1]] are equivalent</nowiki>
	...
	cmd += '&autosummary='+encodeURIComponent(x.summary + pg.wiki.siteSummary());
...
function redirLink(redirMatch, article) {
	...
	if (getValueOf('popupAppendRedirNavLinks') && getValueOf('popupNavLinks')) {
		...
		return ret;
			 ...
			 titledWikiLink({article: new Title().fromWikiText(redirMatch, article.site), action: 'view',  /* FIXME: newWin */
...
function getHistory(wikipage, onComplete) {
	...
	//var url = pg.wiki.titlebasehead + article.site + pg.wiki.apibasetail + '?format=json&formatversion=2&action=query&prop=revisions&titles=';
	//if (pg.re.specialwikisitelink.test(article.site)){
	//	url = (pg.wiki.titlebasehead + article.site + "/api.php?format=json&formatversion=2&action=query&prop=revisions&titles=");
	//}
	url += new Title(wikipage).urlString() + '&rvlimit=' + getValueOf('popupHistoryLimit');
...
//</NOLITE>
	...
	newOption('popupAnchorRegexp', '(A|a)nchors?'); //how to identify an anchors template
...
pg.string = {
	...
	'defaultpopupRedlinkSummary': 'Removing link to empty page [[%s]]',
	'defaultpopupFixDabsSummary': 'Disambiguate [[%s]] to [[%s]]',
	'defaultpopupFixRedirsSummary': 'Redirect bypass from [[%s]] to [[%s]]',
	'defaultpopupExtendedRevertSummary': 'Revert to revision dated %s by %s, oldid %s',
	...
	'defaultpopupQueriedRevertSummary': 'Revert to revision $1 dated $2 by $3',
	'defaultpopupRmDabLinkSummary': 'Remove link to dab page [[%s]]',
	'SummaryByPopupWithoutPopupName' : ' using ',
	...
	//a2569875 mod
	'logevent_type_delete': 'delete log',
	'logevent_type_merge': 'merge log',
	'logevent_type_move': 'move log',
	'logevent_type_patrol': 'patrol log',
	'logevent_type_protect': 'protect log',
	'logevent_type_rights': 'rights log',
	'logevent_type_upload': 'upload log',
	'logevent_type_block': 'block log',
	'logevent_type_import': 'import log',
	'logevent_type_tag': 'tag log',
	'logevent_type_stable': 'delete log',
	'logevent_type_pagetriage': 'delete log',
	'logevent_type_contentmodel': 'contentmodel change log',
	'logevent_type_block': 'block log',
	'logevent_type_spamblacklist': 'abuse log',
	'logevent_type_abusefilter': 'abuse log',
	'logevent_type_comments': 'comments log',
	'logevent_type_avatar': 'avatar log',
	'logevent_type_newusers': 'User creation log',
	'logevent_type_thanks': 'Thanks log',
	
	'logevent_change': 'change',
	'logevent_block': 'block',
	'logevent_reblock': 'reblock',
	'logevent_unblock': 'unblock',
	'logevent_delete': 'delete',
	'logevent_event': 'event',
	'logevent_restore': 'restore',
	'logevent_revision': 'revision',
	'logevent_interwiki': 'interwiki',
	'logevent_upload': 'upload',
	'logevent_merge': 'merge',
	'logevent_move_redir': 'move redirect',
	'logevent_patrol': 'mark patrol',
	'logevent_modify': 'modify protect',
	'logevent_move_prot': 'move protect',
	'logevent_protect': 'protect',
	'logevent_unprotect': 'unprotect',
	'logevent_autopromote': 'auto promote',
	'logevent_erevoke': 'erevoke',
	'logevent_rights': 'rights',
	'logevent_overwrite': 'overwrite',
	'logevent_revert': 'revert',
	'logevent_autopatrol': 'auto mark patrol',
	'logevent_move': 'move',
	'logevent_hit': 'hit',
	'logevent_autocreate': 'automatically create',
	'logevent_thank': 'thank',
	//wikis
	'commons_addr': 'Comm',
	'minecraft_addr': 'MCwiki',
	'minecraftwiki_addr': 'MCwiki',
	'moegirl_addr': 'moe',
	'wikipedia_addr': 'WP',
	'wikimedia_addr': 'WM',
	'wikisource_addr': 'WS',
	...
	'wikinews_addr': 'WN',
	'wikivoyage_addr': 'WVo',
	'wikiversity_addr': 'WVe',
	'wiktionary_addr': 'WT',
	'wikidata_addr': 'WD',
	'uncyclopedia_addr': 'UCWP',
	
	'MessageNoLog': 'No matching items in log. ',
	'MessageHistoryError':'History preview failed :-(',
	'MessageLogEventError':'Log Event preview failed :-(',
	'PageDoesntExist':'Page doesn\'t exist.',
	'MessageLogRef':'page log event listed below for reference.',
	'NoTag':'(No Tag)',
	'NumberUnit':'Unit',
	'FlowPage':'Flow talk page',
	'MessageFlowPageError':'Flow Page preview failed :(',
	'OtherWikiSite':'Other'
};
